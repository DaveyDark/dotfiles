...
EXTENSION_DIRECTORY="${HOME}/.config/zed"
...
eval "$(jq -r '
  ...
  "background=\($special.background) foreground=\($special.foreground)
   cursor=\($special.cursor) color0=\($colors.color0) color1=\($colors.color1)18
   ...
   color8=\($colors.color8) color9=\($colors.color9)33 color10=\($colors.color10)
  ...
  "themes": [
    {
      ...
      "style": {
        ...
        "panel.focused_border": "${color3}",
        "pane.focused_border": "${color3}",
        ...
        "editor.active_line.background": "${color9}",
        ...
        "editor.document_highlight.read_background": "${color9}",
